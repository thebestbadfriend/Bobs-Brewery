...
class TabBuilder:
    ...
    def create_favorite_programs_tab(self, tab_control):
    ...
    def create_contacts_tab(self, tab_control):
        contacts_tab = tk.Frame(tab_control)

        contacts_treeview = self.create_contacts_treeview(contacts_tab)
    ...
    def create_contacts_treeview(self, contacts_tab):
        contacts_treeview = btv(contacts_tab)
        contacts_treeview.heading('#0', text='Companies', anchor=tk.W)

        companies = self.bda.execute_db_command('select * from companies')
        if companies:
            for company in companies:
                ...
                self.build_contacts_treeview_company(contacts_treeview, company_details)

        contacts_treeview.add_node('(No Company)')

        return contacts_treeview

    def build_contacts_treeview_company(self, contacts_treeview, company_details):
        ...
        # any individuals in this section. For now though, it is ok that phone numbers belonging to individuals also
        # show up here. I might even make it toggleable rather than strictly the other way. Maybe some people like it
        # better.
        results = self.bda.execute_db_command(f"""select phone_number
                                                  from phone_numbers
                                                  join contacts_phone_numbers
                                                  on phone_numbers.id = contacts_phone_numbers.phone_number_id
                                                  where
                                                  contacts_phone_numbers.contact_id = {company_details['contact_id']}""")
        if results:
        ...
        results = self.bda.execute_db_command(f"""select fax_number
                                             from fax_numbers
        ...
        results = self.bda.execute_db_command(f"""select email_address
                                             from email_addresses
                                             join contacts_email_addresses
                                             on email_addresses.id = contacts_email_addresses.email_address_id
                                             where
                                             contacts_email_addresses.contact_id = {company_details['contact_id']}""")
        if results:
            for result in results:
        ...
        results = self.bda.execute_db_command(f"""select url
                                             from websites
        ...
        results = self.bda.execute_db_command(f"""select street_address, po_box, city, state, zip_code
                                             from addresses
